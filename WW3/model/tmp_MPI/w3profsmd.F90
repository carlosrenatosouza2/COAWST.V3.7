...
!/
   ...
   USE W3GDATMD, ONLY : REFPARS
       ...
       REAL,    INTENT(IN)    :: DT                    ! Time intervall for which the advection should be computed for the given vel
...
!
           ...
           IF (LCALC) THEN ! If the current field or water level changes estimate the iteration number based on the new flow field a
             KKSUM = 0.d0
             DO IE = 1, NTRI
               NI = TRIGP(IE,:)
...
!
         ...
         IF (LCALC) THEN ! If the current field or water level changes estimate the iteration number based on the new flow field and
           KKSUM = 0.d0
           DO IE = 1, NTRI
             NI = TRIGP(IE,:)
             KKSUM(NI) = KKSUM(NI) + KELEM(:,IE)
