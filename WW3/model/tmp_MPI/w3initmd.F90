...
!/
      ...
      CHARACTER(LEN=512), PARAMETER  :: SWITCHES  = &
                    'F90 NOGRB NC4 DIST MPI PR3 UQ FLX0 LN1 ST4 STAB0 NL1 BT0 DB0 MLIM TR0 BS0 IC0 IS0 REF1 XX0 WNT2 WNX1' // &
                    ' RWND CRT1 CRX1 IG1 TRKNC COAWST O0 O1 O2 O2a O2b O2c O3 O4 O5 O6 O7' // &
                    ''
!/
      CONTAINS
!/ ------------------------------------------------------------------- /
...
!
      IF ( FLOUT(4) .OR. FLOUT(8) ) THEN
          IF (OARST) THEN
            ALLOCATE ( OUTPTS(IMOD)%OUT4%IRQRS(30*NAPROC) )
          ELSE
...
!
              END DO
!
              IF (OARST) CALL W3SETA ( IMOD, NDSE, NDST )
            END IF
          END IF
!
          NRQRS  = IH
