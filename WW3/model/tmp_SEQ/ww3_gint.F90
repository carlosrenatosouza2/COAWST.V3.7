...
!/
        ...
        USE W3WDATMD, ONLY : WDATAS, TIME, WLV, ICE, ICEH, ICEF, UST, USTDIR, ASF
...
!
        ...
        CALL NEXTLN ( COMSTR, NDSI, NDSE )
        READ (NDSI,'(I1)',END=2001,ERR=2002) EXTRA
...
!
          ...
          CALL W3SETW(NG, 6, 6)
!
          CALL W3EXGI ( NG-1, NSEA, NOSWLL_MIN )
!
          CALL TICK21 ( TOUT , DTREQ )
...
!
 ...
 905    FORMAT ( '    Grid Particulars are : '/                                &
                 ...
                 '    Grid Closure = ',I3,'  ==> 0 None, 1 Simple, 2 Tripolar')
 ...
 908    FORMAT (/'  Preparing interpolation weights for output grid ' /        &
                 '  Total number of wet points for interpolation ',I6/)
...
!/
        CONTAINS
!/ -----------------------------------------------------------------------/
        SUBROUTINE W3EXGI ( NGRD, NSEA, NOSWLL_MIN )
!/                  +-----------------------------------+
!/                  | WAVEWATCH-III           NOAA/NCEP |
!/                  |             A. Chawla             |
!/                  |                        FORTRAN 90 |
...
!/ Parameter List
!/
        INTEGER, INTENT(IN) :: NGRD, NSEA, NOSWLL_MIN
!/
!/ Local Parameters
!/
        INTEGER       :: ISEA, GSEA, IG, IGRID, IPTS, IGX, IGY, IX,    &
                         IY, ISWLL, ICAP, IBED, IFREQ, IK
...
!
                  IF ( FLOGRD(8,2) .AND. ACTIVE ) THEN
                    IF ( WADATS(IGRID)%MSCX(GSEA) .NE. UNDEF ) THEN
                      ...
                      MSCXAUX = MSCXAUX + WADATS(IGRID)%MSSX(GSEA)*WT
                      MSCYAUX = MSCYAUX + WADATS(IGRID)%MSSY(GSEA)*WT
                    END IF
                  END IF
!
