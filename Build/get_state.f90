...
!
      CALL wclock_on (ng, IDmod, 80, 198, MyFile)
!
...
!
      ...
      IF (FoundError(exit_flag, NoError, 232, MyFile)) THEN
...
!
     ...
     &                     nVarDim =  nvdim,                            &
     &                     nVarAtt = nvatts)
      IF (FoundError(exit_flag, NoError, 271, MyFile)) RETURN
!
...
!
     ...
     &                          OCEAN(ng) % zeta(:,:,Tindex))
            END IF
            IF (FoundError(status, nf90_noerr, 586, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % rmask,                         &
     &                        OCEAN(ng) % rzeta)
            IF (FoundError(status, nf90_noerr, 639, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                          OCEAN(ng) % ubar(:,:,Tindex))
            END IF
            IF (FoundError(status, nf90_noerr, 713, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                            OCEAN(ng) % t(:,:,:,Tindex,itrc))
              END IF
              IF (FoundError(status, nf90_noerr, 1210, MyFile)) THEN
                IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % rmask,                         &
     &                        MIXING(ng) % AKv)
            IF (FoundError(status, nf90_noerr, 1415, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % rmask,                         &
     &                        MIXING(ng) % AKt(:,:,:,itemp))
            IF (FoundError(status, nf90_noerr, 1475, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % rmask,                         &
     &                        MIXING(ng) % AKt(:,:,:,isalt))
            IF (FoundError(status, nf90_noerr, 1536, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % rmask,                         &
     &                        MIXING(ng) % tke)
            IF (FoundError(status, nf90_noerr, 1766, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % rmask,                         &
     &                        MIXING(ng) % gls)
            IF (FoundError(status, nf90_noerr, 1819, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % rmask,                         &
     &                        MIXING(ng) % Lscale)
            IF (FoundError(status, nf90_noerr, 1872, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % rmask,                         &
     &                        MIXING(ng) % Akk)
            IF (FoundError(status, nf90_noerr, 1925, MyFile)) THEN
              IF (Master) THEN
...
!
     ...
     &                        GRID(ng) % rmask,                         &
     &                        MIXING(ng) % Akp)
            IF (FoundError(status, nf90_noerr, 1979, MyFile)) THEN
              IF (Master) THEN
...
!
      CALL wclock_off (ng, IDmod, 80, 7613, MyFile)
!
