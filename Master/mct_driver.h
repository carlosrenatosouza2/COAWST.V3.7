...
#endif
#ifdef IOMODEL
      !CR IOM PE first and last eval:
      peIOM_frst=pelast+1
      peIOM_last=peIOM_frst+NnodesIOM-1
      pelast=peIOM_last
#endif
...
#ifdef IOMODEL
         !CR:
          WRITE (stdout,23) peIOM_frst, peIOM_last
 23       FORMAT (/,7x,'IOM Model MPI nodes: ',i3.3,' - ', i3.3)
#endif
...
#ifdef IOMODEL
      !CR:
      IF ((peIOM_frst.le.MyRank).and.(MyRank.le.peIOM_last)) THEN
        MyColor=IOMcolor
      END IF
#endif
...
#ifdef IOMODEL
      !CR:
      IF (MyColor.eq.IOMcolor) THEN
        CALL IOmessage(MyCOMM)
       END IF
#endif
#ifdef HYDRO_COUPLING
