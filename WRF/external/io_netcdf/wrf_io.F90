...
!print*, "CR: module wrf_io.f, sub ext_ncd_open_for_write_begin(), antes de NF_CREATE 1"
  stat = NF_CREATE(FileName, NF_CLOBBER, DH%NCID)
#else
  !print*, "CR: module wrf_io.f, sub ext_ncd_open_for_write_begin(), antes de NF_CREATE 2"
  stat = NF_CREATE(FileName, IOR(NF_CLOBBER,NF_64BIT_OFFSET), DH%NCID)
#endif
  else
  create_mode = nf_netcdf4
  !print*, "CR: module wrf_io.f, sub ext_ncd_open_for_write_begin(), antes de NF_CREATE 3"
  stat = NF_CREATE(FileName, create_mode, DH%NCID)
  stat = NF_SET_CHUNK_CACHE(cache_size, cache_nelem, cache_preemption)
  endif
#else
#ifdef WRFIO_NCD_NO_LARGE_FILE_SUPPORT
!print*, "CR: module wrf_io.f, sub ext_ncd_open_for_write_begin(), antes de NF_CREATE 4"
  stat = NF_CREATE(FileName, NF_CLOBBER, DH%NCID)
#else
