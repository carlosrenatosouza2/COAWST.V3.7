...
'start_domain: Idealized case cannot have a separate nested input file' )
      ...
      IF ( grid%c1f(1) .NE. 1. ) THEN
         ...
         IF ( grid%hybrid_opt .NE. 0 ) THEN
            CALL wrf_error_fatal3("<stdin>",629,&
  ...
  ips, ipe, jps, jpe, kps, kpe )
   ...
   DO ij = 1, grid%num_tiles
     ...
     CALL phy_init (  grid%id , config_flags, grid%DT, grid%RESTART, grid%znw, grid%znu,   &
                      ...
                      z_at_q, grid%alt, grid%qnwfa2d, scalar(ims,kms,jms,1), num_scalar,         & 
                      grid%re_cloud, grid%re_ice, grid%re_snow,         & 
                      grid%has_reqc, grid%has_reqi, grid%has_reqs,      & 
                      grid%re_cloud_gsfc, grid%re_ice_gsfc,             & 
                      grid%re_snow_gsfc, grid%re_graupel_gsfc,          & 
                      grid%re_hail_gsfc, grid%re_rain_gsfc,             & 
                      ...
                      config_flags%num_urban_ndm,                           & 
                      config_flags%urban_map_zrd,                           & 
                      config_flags%urban_map_zwd,                           & 
                      config_flags%urban_map_gd,                            & 
                      config_flags%urban_map_zd,                            & 
                      config_flags%urban_map_zdf,                           & 
                      config_flags%urban_map_bd,                            & 
                      config_flags%urban_map_wd,                            & 
                      config_flags%urban_map_gbd,                           & 
                      config_flags%urban_map_fbd,                           & 
