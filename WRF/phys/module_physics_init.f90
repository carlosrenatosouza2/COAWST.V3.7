...
CONTAINS
   ...
   SUBROUTINE phy_init ( id, config_flags, DT, restart, zfull, zhalf,     &
                         ...
                         z_at_q, inv_dens, qnwfa2d, scalar, num_sc,  & 
                         ...
                         ,QR_CU,QS_CU,NC_CU,NI_CU,NR_CU,NS_CU,CCN_CU              & 
                         ,alevsiz_cu,num_months,no_src_types_cu,aeromcu,aeropcu   & 
                         ,EFCG,EFCS,EFIG,EFIS,EFSG,EFSS                           & 
                          )
   ...
   REAL,  DIMENSION( ims:ime , kms:kme , jms:jme )         ,         &
          OPTIONAL, INTENT(INOUT   ) ::                              &
                               QR_CU, QS_CU,                         & 
                               NC_CU, NI_CU, NR_CU, NS_CU,           & 
                               CCN_CU, EFCG, EFCS, EFIG,             & 
                               EFIS, EFSG, EFSS
   ...
   REAL,     DIMENSION(ims:ime,kms:kme,jms:jme), INTENT(IN) :: inv_dens          
  ...
  REAL,     DIMENSION( ims:ime , kms:kme , jms:jme ) , INTENT(OUT), OPTIONAL :: &
   ...
   REAL,    OPTIONAL, DIMENSION(ims:ime,60,jms:jme) :: gecros_state      
...
'--- ERROR: FARMS (swint_opt==2) requires a different MP scheme (Please see the module_physics_init')
      END IF
   END IF
        ...
        DO ls=1,luseas
          ...
          DO LC=1,LUCATS
            IF(found_lu)THEN
              ...
              IF(LC.NE.LI)CALL wrf_error_fatal3("<stdin>",1711,&
...
'arguments not present for calling camzmscheme' )
  ...
  INTEGER :: i,j,itf,jtf
   ...
   mp_select: SELECT CASE(config_flags%mp_physics)
     ...
     CASE (P3_1CATEGORY)
         CALL p3_init('.',1,'WRF',i_err,.false.)
     CASE (P3_1CATEGORY_NC)
         CALL p3_init('.',1,'WRF',i_err,.false.)
     CASE (P3_2CATEGORY)
         CALL p3_init('.',2,'WRF',i_err,.false.)
     CASE (MILBRANDT2MOM)
...
subroutine aerosol_in_cu(aeromcu,alevsiz,no_months,no_src_types,XLAT,XLONG,aeropcu, &
   ...
   INTENT(OUT)  :: aeropcu 
   real, dimension(:, :, :, :, :), allocatable  :: aeroin 
   real, dimension(:, :, :, :), allocatable  :: aeropin 
   CHARACTER (len=10), dimension(no_src_types) :: species 
   CHARACTER (len=100) :: fname 
   CHARACTER (len=8) :: frmt 
   CHARACTER (len=2) :: mons 
   integer :: v, mi 
   REAL, dimension(:), ALLOCATABLE :: lon_aer, lat_aer 
   LOGICAL           :: exists
